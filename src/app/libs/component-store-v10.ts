...
import { concatMap, distinctUntilChanged, map, shareReplay, take, takeUntil, withLatestFrom } from 'rxjs/operators'
...
@Injectable()
export class ComponentStoreV10<T extends object> implements OnDestroy {
  ...
  >(updaterFn: (state: T, value: OriginType) => T): ReturnType {
    return (
      (((observableOrValue?: OriginType | Observable<OriginType>): Subscription => {
  ...
  patchState(partialStateOrUpdaterFn: Partial<T> | ((state: T) => Partial<T>)): void {
    this.setState((state) => {
      const patchedState =
        typeof partialStateOrUpdaterFn === 'function' ? partialStateOrUpdaterFn(state) : partialStateOrUpdaterFn
  ...
  select<O extends Array<Observable<unknown> | SelectConfig | ProjectorFn>, R, ProjectorFn = (...a: unknown[]) => R>(
    ...args: O
  ): Observable<R> {
